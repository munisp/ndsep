...
import { deleteSupervisorFilterPreset, loadSupervisorFilterPresets, saveSupervisorFilterPreset, shareSupervisorFilterPreset, type SupervisorFilterPreset } from "@/lib/supervisor-filter-presets";
...
export default function OperationsScreen() {
  ...
  return (
    <ScreenContainer className="bg-background">
      ...
      </ScrollView><Modal visible={Boolean(acknowledgementTarget)} animationType="slide" onRequestClose={() => setAcknowledgementTarget(null)}><ScreenContainer className="bg-background"><View className="flex-1 justify-end p-5"><View className="rounded-3xl border border-border bg-surface p-5"><Text className="text-xl font-bold text-foreground">Management acknowledgement</Text><Text className="mt-2 text-sm text-muted">Record an internal acknowledgement or resolution note. This does not validate the underlying field observation.</Text><TextInput value={acknowledgementNote} onChangeText={setAcknowledgementNote} placeholder="Required management resolution note" placeholderTextColor="#94A3B8" multiline className="mt-4 min-h-[120px] rounded-xl border border-border bg-background px-3 py-3 text-foreground" /><Pressable onPress={() => acknowledgementTarget && acknowledgementNote.trim().length >= 3 && acknowledgementMutation.mutate({ id: acknowledgementTarget, status: "acknowledged", note: acknowledgementNote.trim() })}><View className="mt-3 rounded-xl bg-foreground px-4 py-4"><Text className="text-center font-semibold text-background">Acknowledge escalation</Text></View></Pressable><Pressable onPress={() => acknowledgementTarget && acknowledgementNote.trim().length >= 3 && acknowledgementMutation.mutate({ id: acknowledgementTarget, status: "resolved", note: acknowledgementNote.trim() })}><View className="mt-3 rounded-xl border border-success px-4 py-4"><Text className="text-center font-semibold text-success">Mark escalation resolved</Text></View></Pressable><Pressable onPress={() => setAcknowledgementTarget(null)}><View className="mt-3 px-4 py-3"><Text className="text-center font-semibold text-muted">Cancel</Text></View></Pressable></View></View></ScreenContainer></Modal>
    </ScreenContainer>
  );
}
