...
export function useMobilePlatformBundle() {
  ...
  return {
    ...
    submitBusinessProfile: async (profile: BusinessProfileRecord) => {
      ...
      if (preference.wifiOnlyAllQueuedMutations && network.type === Network.NetworkStateType.CELLULAR) { const queued = await enqueueStakeholderSubmission({ kind: "profile", profile }); await prependActivity({ title: "Profile deferred for Wi-Fi", description: "The encrypted profile was queued locally because Wi-Fi-only queued mutations are enabled.", category: "onboarding", tone: "warning", route: "/onboarding" }); return { result: null, queuedOffline: true, queueId: queued.id }; }
    ...
    submitStakeholderDocument: async (kind: "identity_document" | "business_document", input: { type: string; fileName: string; mimeType: string; base64Data: string }) => {
      const network = await Network.getNetworkStateAsync(); const preference = await getStakeholderSyncPreferences();
      if ((preference.wifiOnlyDocumentUpload || preference.wifiOnlyAllQueuedMutations) && network.type === Network.NetworkStateType.CELLULAR) { const queued = await enqueueStakeholderSubmission({ kind, ...input }); await prependActivity({ title: "Document deferred for Wi-Fi", description: "The encrypted document was queued locally because Wi-Fi-only queued mutations are enabled.", category: "onboarding", tone: "warning", route: "/onboarding" }); return { result: null, queuedOffline: true, queueId: queued.id }; }
