...
function digestFromReference(reference) {
  const match =
    typeof reference === "string"
      ? reference.match(SHA256_IMAGE_REFERENCE)
      : null;
  return match?.[1] ?? null;
}
...
function hasOnlyPassedChecks(record, label, errors) {
  requireCondition(
    ...
    `${label}: checks must be a non-empty array`
  );
...
function verifyCandidate(candidate, errors) {
  ...
  requireCondition(
    errors,
    digest !== null,
    "candidate.json: image must be a lower-case immutable OCI sha256 reference"
  );
  requireCondition(
    errors,
    COMMIT_SHA.test(candidate?.sourceCommit ?? ""),
    "candidate.json: sourceCommit must be a full lower-case commit SHA"
  );
  requireCondition(
    errors,
...
function verifyTrivy(trivy, digest, errors) {
  requireCondition(
    errors,
    trivy?.scanTarget?.endsWith(`@sha256:${digest}`),
    "trivy.json: scanTarget must be the candidate image digest"
  );
  requireCondition(
    errors,
    trivy?.scanner === "trivy",
    "trivy.json: scanner must identify Trivy"
  );
  requireCondition(
    errors,
    trivy?.mode === "direct-image",
    "trivy.json: mode must be direct-image"
  );
  const findings =
  ...
  requireCondition(
    errors,
    blocked.length === 0,
    `trivy.json: found ${blocked.length} HIGH/CRITICAL vulnerability finding(s)`
  );
}
...
function verifyArtifactEvidence(artifact, digest, errors) {
  for (const [name, report] of Object.entries(artifact ?? {})) {
    requireCondition(
      ...
      `artifact-evidence.json: ${name} is not bound to the candidate digest`
    );
    requireCondition(
      errors,
    ...
    requireCondition(
      errors,
      typeof report?.uri === "string" && report.uri.length > 0,
      `artifact-evidence.json: ${name} is missing immutable evidence URI`
    );
  }
  requireCondition(
    errors,
    ["SPDX", "CycloneDX"].includes(artifact?.sbom?.format),
    "artifact-evidence.json: an SPDX or CycloneDX SBOM is required"
  );
  requireCondition(
    errors,
    artifact?.provenance?.format === "SLSA",
    "artifact-evidence.json: SLSA provenance is required"
  );
  requireCondition(
    errors,
    ...
    "artifact-evidence.json: cosign certificate identity must bind to the production release workflow"
  );
}
...
function verifyGovernance(governance, errors) {
  requireCondition(
    ...
    "governance.json: snapshot must describe production"
  );
  requireCondition(
    errors,
    governance?.requiredApprovals >= 2,
    "governance.json: production requires at least two approvals"
  );
  requireCondition(
    errors,
  ...
  requireCondition(
    errors,
    governance?.dismissStaleReviews === true,
    "governance.json: stale reviews are not dismissed"
  );
  requireCondition(
    errors,
    governance?.requireLastPushApproval === true,
    "governance.json: last-push approval is not required"
  );
  requireCondition(
    errors,
    governance?.strictStatusChecks === true,
    "governance.json: strict status checks are not enabled"
  );
  requireCondition(
    errors,
    governance?.enforceAdmins === true,
    "governance.json: administrators can bypass controls"
  );
  requireCondition(
    errors,
    governance?.allowForcePushes === false,
    "governance.json: force pushes are allowed"
  );
  requireCondition(
    errors,
    governance?.allowDeletions === false,
    "governance.json: branch deletion is allowed"
  );
  requireCondition(
    errors,
    governance?.codeownersErrors?.length === 0,
    "governance.json: CODEOWNERS contains unresolved errors"
  );
  requireCondition(
    errors,
    governance?.productionReleaseEnvironment?.protected === true,
    "governance.json: production-release environment is not protected"
  );
  requireCondition(
    errors,
    governance?.productionReleaseEnvironment?.preventSelfReview === true,
    "governance.json: production-release permits self-review"
  );
}
...
function verifyStaging(staging, digest, errors) {
  requireCondition(
    ...
    "staging-deployment.json: evidence must be from protected staging"
  );
  requireCondition(
    errors,
    staging?.protectedEnvironment === true,
    "staging-deployment.json: staging environment is not protected"
  );
  requireCondition(
    errors,
    staging?.deploymentDigest === digest,
    "staging-deployment.json: deployed digest does not match candidate"
  );
  requireCondition(
    errors,
    staging?.status === "passed",
    "staging-deployment.json: deployment status is not passed"
  );
  requireCondition(
    errors,
    staging?.mtlsContract === "passed",
    "staging-deployment.json: mTLS contract test is not passed"
  );
  requireCondition(
    errors,
    staging?.authorizationNegativePath === "passed",
    "staging-deployment.json: authorization negative-path test is not passed"
  );
...
function verifyServices(matrix, digest, errors) {
  const rows = new Map(
    (matrix?.services ?? []).map(service => [service?.name, service])
  );
  for (const name of REQUIRED_SERVICES) {
    ...
    requireCondition(
      errors,
      service !== undefined,
      `service-matrix.json: missing '${name}' evidence`
    );
    requireCondition(
      errors,
      service?.status === "passed",
      `service-matrix.json: ${name} is not passed`
    );
    requireCondition(
      errors,
      service?.digest === digest,
      `service-matrix.json: ${name} is not bound to candidate digest`
    );
    requireCondition(
      errors,
      typeof service?.evidenceUri === "string" &&
        service.evidenceUri.length > 0,
      `service-matrix.json: ${name} is missing evidence URI`
    );
  }
}
...
function verifyOperationalEvidence(postgres, resilience, residency, errors) {
  for (const [label, record, requiredChecks] of [
    ...
    [
      "resilience.json",
      resilience,
      ["stateful-service-fault-recovery", "alert-escalation", "reconciliation"],
    ],
    [
    ...
    requireCondition(
      ...
      `${label}: overall status is not passed`
    );
    const names = new Set(
      (record?.checks ?? [])
    ...
    for (const check of requiredChecks)
      requireCondition(
        errors,
        names.has(check),
        `${label}: required check '${check}' is absent or failed`
      );
  }
}
...
function verifyAcceptances(approvals, digest, errors) {
  requireCondition(
    ...
    "approvals.json: approvals are not bound to candidate digest"
  );
  const roles = new Set(
    (approvals?.acceptances ?? [])
      .filter(acceptance => acceptance?.decision === "accepted")
      .map(acceptance => acceptance.role)
  );
  for (const role of REQUIRED_ACCEPTANCE_ROLES)
    requireCondition(
      errors,
      roles.has(role),
      `approvals.json: missing accepted '${role}' sign-off`
    );
}
...
function parseArguments(argumentsList) {
  ...
  if (position === -1 || !argumentsList[position + 1]) {
    throw new Error(
      "Usage: node scripts/ci/verify-60-readiness-evidence.mjs --evidence-dir /secure/path/to/evidence"
    );
  }
...
if (
  ...
  try {
    const result = verifyReadiness60Evidence(
