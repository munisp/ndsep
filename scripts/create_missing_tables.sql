-- Create missing tables that are in schema.ts but not in local postgres

-- Enums (create if not exists)
DO $$ BEGIN
  CREATE TYPE IF NOT EXISTS lawful_basis AS ENUM ('consent','contract','legal_obligation','vital_interests','public_task','legitimate_interests');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  CREATE TYPE IF NOT EXISTS consent_status AS ENUM ('active','withdrawn','expired','pending');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  CREATE TYPE IF NOT EXISTS breach_severity AS ENUM ('low','medium','high','critical');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  CREATE TYPE IF NOT EXISTS breach_status AS ENUM ('detected','assessing','ndpc_notified','individuals_notified','contained','resolved','closed');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  CREATE TYPE IF NOT EXISTS dpo_credential_status AS ENUM ('pending','verified','expired','suspended','revoked');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  CREATE TYPE IF NOT EXISTS dpia_status AS ENUM ('draft','in_progress','review','approved','rejected','archived');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
DO $$ BEGIN
  CREATE TYPE IF NOT EXISTS dpia_risk_level AS ENUM ('low','medium','high','critical');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
