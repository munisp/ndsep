  ...
  try {
    ...
    if (!FORCE) {
      ...
      if (parseInt(rows[0].c) > 0) {
        console.log(
          "⚠️  Database already has data. Use FORCE_SEED=1 to re-seed."
        );
        return;
      }
    }
    ...
    `);
    console.log("✅ Nigerian organizations seeded (10)");
    ...
    await client.query(`
      ...
      VALUES
        ...
        ('Audit Log Retention 7 Years',
         ...
         'medium', true, 1.0)
    `);
    console.log("✅ NDPR compliance policies seeded (8)");
    ...
    await client.query(`
      ...
      VALUES
        (2, 1, 6,  'MTN Nigeria: PII Stored Outside Nigeria',
         'Azure Blob Storage in westeurope contains 78M subscriber PII records violating NDPR Section 2.6',
         'critical', 'non_compliant', 'penalty_imposed', NOW() - INTERVAL '5 days', 50000000),
        (5, 1, 11, 'NNPC: Financial Data on Foreign Cloud',
         'GCP Analytics bucket in europe-west1 contains 2.3M financial records in violation of NDPR',
    ...
    await client.query(`
      ...
      VALUES
        (2,  5,  'zeek',    'bgp_hijack',         'BGP Route Hijack Detected - AS37148',
         'Unauthorized BGP announcement for 197.255.0.0/16 (MTN Nigeria ASN) from AS37148',
        ...
        (5,  10, 'suricata','data_exfiltration',  'Cross-Border Data Exfiltration Attempt',
         'Large data transfer (2.1GB) from NNPC Warri Refinery SCADA to IP in Netherlands',
        ...
        (3,  7,  'wazuh',   'unauthorized_access','Unauthorized Access to LUTH Patient Records',
         'Multiple failed SSH login attempts on LUTH patient records server from 41.58.123.45',
         'high', false, NULL, 'T1110.001', NOW() - INTERVAL '2 hours'),
        (1,  1,  'wazuh',   'anomaly',            'Suspicious API Activity - FBN Core Banking',
         'Unusual API call pattern on First Bank Nigeria core banking server — possible credential stuffing',
         'high', false, NULL, 'T1078', NOW() - INTERVAL '1 hour'),
        (6,  12, 'suricata','intrusion',          'SQL Injection Attempt on Jumia E-Commerce',
         'SQL injection pattern detected in Jumia e-commerce platform request logs from 41.203.78.12',
         'medium', true, NULL, 'T1190', NOW() - INTERVAL '3 hours'),
        (2,  4,  'wazuh',   'anomaly',            'MTN: TLS Certificate Expiry Warning',
         'MTN Subscriber Management System TLS certificate expires in 7 days',
         'low', false, NULL, NULL, NOW() - INTERVAL '6 hours'),
        (10, 16, 'suricata','intrusion',          'Port Scan Detected on NCC Systems',
         'Systematic port scan against NCC Licensing Management System from 196.46.12.89',
         'medium', false, NULL, 'T1046', NOW() - INTERVAL '4 hours'),
        (9,  15, 'wazuh',   'anomaly',            'Zenith Bank: Unusual Transaction Volume',
         'Transaction volume 340% above baseline on Zenith Internet Banking Platform',
         'high', false, NULL, 'T1499', NOW() - INTERVAL '30 minutes')
    `);
    console.log("✅ Security alerts seeded (8)");

    // ── Network Events ──────────────────────────────────────────────────────
    // Note: bytes_transferred is int4 (max 2,147,483,647)
    await client.query(`
      ...
      VALUES
        (2,  '192.168.10.5', '20.50.80.100',  'Nigeria', 'Netherlands', 6.4541, 3.3947, 52.3667,  4.9000, 'HTTPS', 443, 2100000000, 'exfiltration_attempt', true,  'IXPN-Lagos', 'ET-POLICY-CLOUD-UPLOAD', false, NOW() - INTERVAL '45 minutes'),
    ...
    `);
    console.log("✅ Network events seeded (8)");
    ...
    await client.query(`
      ...
      VALUES
        (2, 1, 50000000,  'NGN', 'pending',    'TB-NITDA-2024-MTN-001',  NOW() + INTERVAL '26 days', 'NDPR Section 2.6: PII outside Nigeria on Azure westeurope. First offence. Ref: NITDA-PEN-2024-MTN-001'),
        (5, 5, 100000000, 'NGN', 'overdue',    'TB-NITDA-2024-NNPC-001', NOW() - INTERVAL '9 days',  'NDPR critical vulnerability SLA breach on NNPC SCADA. Ref: NITDA-PEN-2024-NNPC-001'),
        ...
        (3, 8, 25000000,  'NGN', 'processing', 'TB-NITDA-2024-LUTH-001', NOW() + INTERVAL '25 days', 'NDPR health data encryption non-compliance at LUTH. Ref: NITDA-PEN-2024-LUTH-001'),
    ...
    `);
    console.log("✅ Financial penalties seeded (5) — Total: ₦225,000,000 NGN");
    ...
    `);
    console.log("✅ Threat intelligence seeded (5)");
    ...
    await client.query(`
      ...
      VALUES
        (1,  'FBN Customer PII Database',      'First Bank Nigeria customer data: BVN, NIN, account details, transaction history',          'structured', 'tier1_pii',       'Victoria Island DC - PostgreSQL Primary',   true,  4500000,  94.2),
        ...
        (3,  'LUTH Patient Health Records',    'Lagos University Teaching Hospital patient medical records and diagnostic imaging',          'structured', 'tier3_health',    'LUTH Server Room - Patient Records Server',  true,  320000,   91.8),
        (4,  'FME Student Enrollment Data',    'Federal Ministry of Education national student enrollment and academic records',             'structured', 'tier1_pii',       'Abuja Ministry HQ - EMIS System',            true,  12500000, 89.3),
        ...
        (10, 'NCC Spectrum License Registry',  'NCC spectrum allocation, license registry, and renewal records',                           'structured', 'tier4_government','NCC HQ Abuja - Licensing Management System', true,  45000,    99.1)
    `);
    console.log("✅ Data catalog entries seeded (8)");
    ...
    await client.query(`
      ...
      VALUES
        (2,  'ndsep-risk-v2.1', 74.4, 78.4, 0.89, 30, '{"cross_border_data":1,"agent_timeout":1,"bgp_anomaly":1}',                                    'Resolve Azure westeurope data residency violation and investigate BGP anomaly'),
        (5,  'ndsep-risk-v2.1', 89.3, 91.2, 0.94, 30, '{"critical_vulnerabilities":12,"cross_border_data":1,"undeclared_assets":142,"scada":1}',       'Critical: patch SCADA CVEs within 72h, repatriate GCP data, declare all assets to NITDA'),
    ...
    `);
    console.log("✅ ML risk predictions seeded (8)");

    // ── Audit Logs ──────────────────────────────────────────────────────────
    await client.query(`
      INSERT INTO audit_logs (user_id, organization_id, action, resource_type, resource_id, details, ip_address)
      VALUES
        (NULL, 2,  'VIOLATION_DETECTED',  'compliance_violation', 1, 'NDPR violation: MTN Nigeria 78M subscriber PII records on Azure westeurope',                     '10.0.0.1'),
        (NULL, 5,  'PENALTY_ISSUED',      'financial_penalty',    2, 'NITDA penalty issued to NNPC: NGN 100,000,000 for SCADA vulnerability SLA breach',               '10.0.0.1'),
        (NULL, 2,  'BGP_HIJACK_DETECTED', 'security_alert',       1, 'BGP route hijack: AS37148 announcing MTN Nigeria prefix 197.255.0.0/16',                         '10.0.0.1'),
        (NULL, 3,  'AUDIT_SCHEDULED',     'organization',         3, 'NITDA compliance audit scheduled for Lagos University Teaching Hospital',                         '10.0.0.1'),
        (NULL, 1,  'ASSET_DISCOVERED',    'asset',                3, 'Cloud asset discovered: AWS S3 (us-east-1) with tier1_pii classification outside Nigeria',        '10.0.0.1'),
        (NULL, 8,  'NOTICE_ISSUED',       'enforcement_action',   4, 'NDSEP notice issued to Nigerian Ports Authority for agent version non-compliance (v2.0.8)',       '10.0.0.1'),
        ...
        (NULL, 5,  'VIOLATION_ESCALATED', 'compliance_violation', 5, 'NNPC SCADA vulnerability escalated to critical — NGN 100M penalty imposed by NITDA',             '10.0.0.1'),
    ...
    `);
    console.log("✅ Audit logs seeded (10)");

    console.log("\n🎉 NDSEP database seeded successfully with Nigerian demo data!");
    const counts = await client.query(`
      SELECT
        ...
        (SELECT COUNT(*) FROM security_alerts)     as alerts,
        (SELECT COUNT(*) FROM network_events)      as network_events,
        (SELECT COUNT(*) FROM financial_penalties) as penalties,
        (SELECT COUNT(*) FROM threat_intelligence) as threat_intel,
        (SELECT COUNT(*) FROM data_catalog_entries) as catalog_entries,
        (SELECT COUNT(*) FROM ml_risk_predictions) as ml_predictions,
        (SELECT COUNT(*) FROM audit_logs)          as audit_logs
    `);
    console.table(counts.rows[0]);
    console.log("\n🏛️  Nigerian Organizations:");
    console.log("   • First Bank of Nigeria Plc (Lagos) — Financial Services");
    console.log("   • MTN Nigeria Communications Plc (Lagos) — Telecommunications");
    console.log("   • Lagos University Teaching Hospital (Lagos) — Healthcare");
    console.log("   • Federal Ministry of Education (Abuja) — Government");
    console.log("   • Nigerian National Petroleum Corp (Abuja) — Energy");
    console.log("   • Jumia Technologies AG Nigeria (Lagos) — E-Commerce");
    console.log("   • AIICO Insurance Plc (Lagos) — Insurance");
    console.log("   • Nigerian Ports Authority (Lagos) — Transportation");
    console.log("   • Zenith Bank Plc (Lagos) — Financial Services");
    console.log("   • Nigerian Communications Commission (Abuja) — Government");
    console.log("\n💰 Total Penalties Issued: ₦225,000,000 NGN");
  } finally {
    client.release();
    await pool.end();
  }
