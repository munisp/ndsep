...
import { recordAuditInvestigationExport, recordKeycloakSessionRevocation, verifyAuditInvestigationExportReceipt, verifySecurityAuditChain } from "../securityOperations";
import { validateBulkSessionRevocation } from "../../lib/keycloak-bulk-revocation";
import { deleteTeamWafThreatPreset, listTeamWafThreatPresetRevisions, listTeamWafThreatPresets, publishTeamWafThreatPreset, reviewTeamWafThreatPresetRevision, submitTeamWafThreatPresetRevision } from "../wafThreatPresetRepository";
...
export const systemRouter = router({
  ...
  verifyAuditInvestigationExportReceipt: publicProcedure.input(z.object({ eventId: z.string().uuid() })).query(({ input }) => verifyAuditInvestigationExportReceipt(input.eventId)),
  ...
  listTeamWafThreatPresetRevisions: adminProcedure.input(z.object({ presetId: z.string().uuid() })).query(async ({ ctx, input }) => { if (!ctx.enterprise) throw new Error("Verified enterprise identity with an agency claim is required for team presets."); return listTeamWafThreatPresetRevisions({ presetId: input.presetId, agencyId: ctx.enterprise.agencyId }); }),
  submitTeamWafThreatPresetRevision: adminProcedure.input(z.object({ presetId: z.string().uuid(), query: z.string().trim().min(1).max(160) })).mutation(async ({ ctx, input }) => { if (!ctx.enterprise) throw new Error("Verified enterprise identity with an agency claim is required for team presets."); return submitTeamWafThreatPresetRevision({ presetId: input.presetId, agencyId: ctx.enterprise.agencyId, query: input.query, actor: ctx.enterprise.subject }); }),
  reviewTeamWafThreatPresetRevision: adminProcedure.input(z.object({ presetId: z.string().uuid(), revisionId: z.string().uuid(), approved: z.boolean(), note: z.string().trim().min(5).max(240) })).mutation(async ({ ctx, input }) => { if (!ctx.enterprise) throw new Error("Verified enterprise identity with an agency claim is required for team presets."); assertEnterpriseRole(ctx.enterprise, ["planning_supervisor"]); return reviewTeamWafThreatPresetRevision({ presetId: input.presetId, agencyId: ctx.enterprise.agencyId, revisionId: input.revisionId, reviewer: ctx.enterprise.subject, approved: input.approved, note: input.note }); }),
