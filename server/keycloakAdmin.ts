...
export function keycloakAdminStatus() {
  try {
    const available = Boolean(configuredAdmin());
    return { available, reason: available ? null : "Keycloak administrative session revocation is not configured." };
  } catch (error) {
    return { available: false, reason: error instanceof Error ? error.message : "Keycloak administrative session revocation configuration is invalid." };
  }
}
...
export async function revokeKeycloakSession(sessionId: string) {
  let config: KeycloakAdminConfig | null;
  try { config = configuredAdmin(); } catch (error) { return { revoked: false, reason: error instanceof Error ? error.message : "Keycloak administrative session revocation configuration is invalid." } as const; }
  if (!config) return { revoked: false, reason: "Keycloak administrative session revocation is not configured." } as const;
