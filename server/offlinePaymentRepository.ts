...
export async function getReconciliationRetryOutcomeTrend(days = 7) { const pool = await readyPool(); const safeDays = Math.max(1, Math.min(days, 31)); const result = await pool.query<{ bucket: string; retry_status: string; count: string }>("SELECT to_char(date_trunc('day', received_at), 'YYYY-MM-DD') AS bucket, retry_status, COUNT(*)::text AS count FROM payment_gateway_webhook_deliveries WHERE received_at >= now() - ($1::text || ' days')::interval AND retry_status <> 'not_scheduled' GROUP BY 1, 2 ORDER BY 1 ASC", [String(safeDays)]); const grouped = new Map<string, Record<string, number>>(); for (const row of result.rows) grouped.set(row.bucket, { ...(grouped.get(row.bucket) ?? {}), [row.retry_status]: Number(row.count) }); return [...grouped.entries()].map(([date, outcomes]) => ({ date, scheduled: outcomes.scheduled ?? 0, succeeded: outcomes.succeeded ?? 0, blocked: outcomes.blocked ?? 0, exhausted: outcomes.exhausted ?? 0 })); }
export async function recordReconciliationExceptionExport(input: { actorOpenId: string; status: string; retryOnly: boolean; rowCount: number }) { return transaction(async (client) => { const at = new Date().toISOString(); const exportId = crypto.randomUUID(); await appendEvent(client, { aggregateType: "payment_audit_export", aggregateId: exportId, eventType: "payment_reconciliation_exceptions_exported", actorOpenId: input.actorOpenId, occurredAt: at, data: input }); return { exportId, occurredAt: at }; }); }
...
export async function acknowledgeHighRiskReconciliationAlert(input: { alertId: string; role: PaymentApprovalRole; actorOpenId: string }) { return transaction(async (client) => { const found = await client.query<RoleAlertRow>("SELECT * FROM payment_reconciliation_role_alerts WHERE id = $1::uuid AND target_role = $2 FOR UPDATE", [input.alertId, input.role]); const alert = found.rows[0]; if (!alert) throw new Error("The high-risk alert was not found for this role."); if (alert.read_at) return mapRoleAlert(alert); const at = new Date().toISOString(); const updated = await client.query<RoleAlertRow>("UPDATE payment_reconciliation_role_alerts SET read_at = $2::timestamptz WHERE id = $1::uuid RETURNING *", [input.alertId, at]); await appendEvent(client, { aggregateType: "gateway_webhook", aggregateId: alert.delivery_id, eventType: "gateway_high_risk_alert_acknowledged", actorOpenId: input.actorOpenId, occurredAt: at, data: { alertId: input.alertId, targetRole: input.role, settlementStatusUnchanged: true } }); return mapRoleAlert(updated.rows[0]!); }); }
