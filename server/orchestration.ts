...
async function callService<T = unknown>(
  ...
  const serviceName =
    Object.entries(ORCHESTRATION_SERVICES).find(
      ([, v]) => v === serviceUrl
    )?.[0] ?? "unknown";
  ...
  try {
    ...
    logger.warn(
      { service: serviceName, url, latencyMs, err: message },
      "[orchestration] Service call failed after retries"
    );
...
export async function j03_violationDetected(params: {
  ...
  const [eventResult, wfResult, lhResult] = await Promise.allSettled([
    ...
    callService(
      ORCHESTRATION_SERVICES.workflowEngine,
      "/workflows/start",
      "POST",
      {
...
export async function j04_penaltyIssued(params: {
  ...
  const tbResult = await callService(
    ORCHESTRATION_SERVICES.tigerBeetle,
    "/ledger/penalty/issue",
    "POST",
    {
      org_id: params.orgId,
      violation_id: params.violationId,
  ...
  const lhResult = await callService(
    ORCHESTRATION_SERVICES.lakehouse,
    "/ingest",
    "POST",
    {
  ...
  return {
    journey: "J04",
    ledger: tbResult.ok ? "posted" : "degraded",
    lakehouse: lhResult.ok ? "ingested" : "degraded",
    tx_id: (tbResult.data as { tx_id?: string })?.tx_id,
  };
}
...
export async function j05_penaltyPaid(params: {
  ...
  return {
    journey: "J05",
    ledger: tbResult.ok ? "settled" : "degraded",
    lakehouse: lhResult.ok ? "ingested" : "degraded",
  };
}
...
export async function j06_transferRequested(params: {
  ...
  const mlResult = await callService<{
    risk_score: number;
    risk_level: string;
    ...
    data_volume_gb: params.volumeGb,
    transfer_purpose: params.purpose,
    recipient_org_type: params.recipientType,
  });
  const riskScore = mlResult.data?.risk_score ?? 50;
  const recommendation = mlResult.data?.recommendation ?? "review";
  const wfResult = await callService(
    ORCHESTRATION_SERVICES.workflowEngine,
    "/workflows/start",
    "POST",
    {
      workflowType: "transfer_approval",
      workflowId: `transfer-${params.transferId}`,
      input: {
  ...
  return {
    ...
    recommendation,
    flags: mlResult.data?.flags ?? [],
    workflow: wfResult.ok ? "started" : "degraded",
    lakehouse: lhResult.ok ? "ingested" : "degraded",
  };
}
...
export async function j14_riskScoreUpdated(params: {
  ...
  penaltyAmountYtd: number;
  transferCount30d: number;
  rejectedTransferRate: number;
  networkAnomalyCount7d: number;
  slaBreach90d: number;
  ...
  const mlResult = await callService<{
    ...
    violation_count_30d: params.violationCount30d,
    penalty_amount_ytd: params.penaltyAmountYtd,
    transfer_count_30d: params.transferCount30d,
    rejected_transfer_rate: params.rejectedTransferRate,
    network_anomaly_count_7d: params.networkAnomalyCount7d,
    sla_breach_count_90d: params.slaBreach90d,
    days_since_last_audit: params.daysSinceLastAudit,
    sector_risk_multiplier: params.sectorRiskMultiplier,
    data_volume_tb: params.dataVolumeTb,
  });
  const riskScore = mlResult.data?.risk_score ?? params.complianceScore;
  await callService(
    ORCHESTRATION_SERVICES.eventBus,
    "/events/publish",
    "POST",
    {
      ...
      payload: {
        orgId: params.orgId,
        riskScore,
        riskLevel: mlResult.data?.risk_level ?? "medium",
        modelVersion: mlResult.data?.model_version ?? "v2.1.0",
        confidence: mlResult.data?.confidence ?? 0.85,
  ...
  return {
    ...
    risk_level: mlResult.data?.risk_level ?? "medium",
    confidence: mlResult.data?.confidence ?? 0.85,
    model_version: mlResult.data?.model_version ?? "v2.1.0",
    ml_service: mlResult.ok ? "computed" : "degraded",
  };
}
...
export async function j18_revenueDistribution(params: {
  ...
  return {
    journey: "J18",
    ledger: tbResult.ok ? "distributed" : "degraded",
...
export async function j20_penaltyDisputed(params: {
  ...
  const tbResult = await callService(
    ORCHESTRATION_SERVICES.tigerBeetle,
    "/ledger/dispute/escrow",
    "POST",
    {
      penalty_id: params.penaltyId,
      org_id: params.orgId,
      amount_usd: params.amountUsd,
      dispute_ref: params.disputeRef,
    }
  );
  const wfResult = await callService(
    ORCHESTRATION_SERVICES.workflowEngine,
    ...
    "POST",
    {
      workflowType: "penalty_dispute",
      workflowId: `dispute-${params.penaltyId}`,
      input: {
  ...
  return {
    journey: "J20",
    escrow: tbResult.ok ? "held" : "degraded",
    workflow: wfResult.ok ? "started" : "degraded",
    escrow_id: (tbResult.data as { escrow_id?: string })?.escrow_id,
  };
}

export async function j25_financialReconciliation() {
  const summaryResult = await callService(
    ORCHESTRATION_SERVICES.tigerBeetle,
    "/ledger/summary",
    "GET"
  );
  if (summaryResult.ok && summaryResult.data) {
    await callService(ORCHESTRATION_SERVICES.lakehouse, "/ingest", "POST", {
      ...
      record: {
        tx_type: "reconciliation_snapshot",
        ledger_id: "tigerbeetle-main",
        debit_account: "reconciliation",
        credit_account: "reconciliation",
        amount: 0,
        currency: "USD",
        reference: `recon-${new Date().toISOString().slice(0, 7)}`,
        occurred_at: new Date().toISOString(),
      },
      journey_id: "J25",
    });
  ...
  return {
    ...
    status: summaryResult.ok ? "reconciled" : "degraded",
  };
}
...
export async function checkOrchestrationHealth(): Promise<ServiceHealth[]> {
  const checks = Object.entries(ORCHESTRATION_SERVICES).map(
    async ([name, url]) => {
