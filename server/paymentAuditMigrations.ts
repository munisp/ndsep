...
const PAYMENT_AUDIT_DDL = [
  ...
  `ALTER TABLE payment_gateway_webhook_deliveries ADD COLUMN IF NOT EXISTS exception_status text NOT NULL DEFAULT 'not_exception'`,
  `ALTER TABLE payment_gateway_webhook_deliveries ADD COLUMN IF NOT EXISTS exception_opened_at timestamptz NULL`,
  `ALTER TABLE payment_gateway_webhook_deliveries ADD COLUMN IF NOT EXISTS exception_resolved_at timestamptz NULL`,
  `ALTER TABLE payment_gateway_webhook_deliveries ADD COLUMN IF NOT EXISTS exception_resolved_by text NULL`,
  `ALTER TABLE payment_gateway_webhook_deliveries ADD COLUMN IF NOT EXISTS exception_resolution_note text NULL`,
  `ALTER TABLE payment_gateway_webhook_deliveries ADD COLUMN IF NOT EXISTS provider_transaction_id text NULL`,
  `ALTER TABLE payment_gateway_webhook_deliveries ADD COLUMN IF NOT EXISTS retry_status text NOT NULL DEFAULT 'not_scheduled'`,
  `ALTER TABLE payment_gateway_webhook_deliveries ADD COLUMN IF NOT EXISTS retry_count integer NOT NULL DEFAULT 0`,
  `ALTER TABLE payment_gateway_webhook_deliveries ADD COLUMN IF NOT EXISTS retry_after timestamptz NULL`,
  `ALTER TABLE payment_gateway_webhook_deliveries ADD COLUMN IF NOT EXISTS retry_last_error text NULL`,
  `ALTER TABLE payment_gateway_webhook_deliveries DROP CONSTRAINT IF EXISTS payment_gateway_webhook_deliveries_retry_status_check`,
  `ALTER TABLE payment_gateway_webhook_deliveries ADD CONSTRAINT payment_gateway_webhook_deliveries_retry_status_check CHECK (retry_status IN ('not_scheduled', 'scheduled', 'in_progress', 'succeeded', 'exhausted', 'blocked'))`,
  ...
  `ALTER TABLE payment_gateway_webhook_deliveries DROP CONSTRAINT IF EXISTS payment_gateway_webhook_deliveries_exception_status_check`,
  `ALTER TABLE payment_gateway_webhook_deliveries ADD CONSTRAINT payment_gateway_webhook_deliveries_exception_status_check CHECK (exception_status IN ('not_exception', 'open', 'resolved', 'dismissed'))`,
  `CREATE INDEX IF NOT EXISTS payment_gateway_webhook_deliveries_exception_idx ON payment_gateway_webhook_deliveries (received_at DESC) WHERE exception_status = 'open'`,
  `CREATE INDEX IF NOT EXISTS payment_gateway_webhook_deliveries_retry_idx ON payment_gateway_webhook_deliveries (retry_after ASC) WHERE retry_status = 'scheduled'`,
  `CREATE TABLE IF NOT EXISTS payment_reconciliation_role_alerts (
      id uuid PRIMARY KEY,
      delivery_id uuid NOT NULL REFERENCES payment_gateway_webhook_deliveries(id),
      target_role text NOT NULL CHECK (target_role IN ('mining_reviewer', 'petroleum_reviewer', 'environment_reviewer', 'planning_supervisor')),
      severity text NOT NULL CHECK (severity IN ('high')),
      title text NOT NULL,
      body text NOT NULL,
      created_at timestamptz NOT NULL,
      read_at timestamptz NULL,
      CONSTRAINT payment_reconciliation_role_alerts_delivery_role_unique UNIQUE (delivery_id, target_role)
    )`,
  `CREATE INDEX IF NOT EXISTS payment_reconciliation_role_alerts_role_idx ON payment_reconciliation_role_alerts (target_role, created_at DESC)`,
