...
async function listDpcoOrganisations(opts: {
  ...
  if (opts.type) {
    conditions.push("organisation_type = ?");
  ...
  const rows = await q(
    `SELECT * FROM dpco_organisations ${where} ORDER BY name ASC LIMIT ? OFFSET ?`,
    [...params, limit, offset]
  );
...
async function getDpcoOrganisation(id: number) {
  ...
  if (!row)
    throw new TRPCError({ code: "NOT_FOUND", message: "DPCO not found" });
  return row;
}
...
async function upsertDpcoOrganisation(data: Record<string, unknown>) {
  if (data.id) {
    await q(
      ...
      [
        ...
        data.ndpcReference,
        data.cacNumber,
        data.taxClearanceVerified ? 1 : 0,
        data.ngDomainVerified ? 1 : 0,
        data.id,
      ]
    );
  ...
  const [result] = await q<any>(
    ...
    [
      ...
      data.licenceExpiresAt,
      data.status ?? "pending",
      data.organisationType ?? "consultancy",
      data.email,
      data.phone,
      data.website,
      data.state,
...
async function listDpcoClients(dpcoOrgId?: number, status?: string) {
  ...
  const params: unknown[] = [];
  if (dpcoOrgId) {
    conditions.push("c.dpco_org_id = ?");
    params.push(dpcoOrgId);
  }
  if (status) {
...
async function upsertDpcoClient(data: Record<string, unknown>) {
  if (data.id) {
    await q(
      ...
      [
        ...
        data.orgLocation,
        data.contactName
          ? encryptField(String(data.contactName))
          : data.contactName,
        data.contactEmail
          ? encryptField(String(data.contactEmail))
          : data.contactEmail,
        data.contactPhone
          ? encryptField(String(data.contactPhone))
          : data.contactPhone,
        data.status,
        data.riskLevel,
        data.id,
  ...
  const [result] = await q<any>(
    ...
    [
      data.dpcoOrganisationId ?? data.dpcoOrgId,
      data.orgName ?? data.organisationName ?? "New Client",
      data.orgSector,
      data.orgLocation,
      data.contactName ? encryptField(String(data.contactName)) : null,
      data.contactEmail ? encryptField(String(data.contactEmail)) : null,
      data.contactPhone ? encryptField(String(data.contactPhone)) : null,
      data.status ?? "active",
...
async function listVerificationStatements(opts: {
  dpcoOrgId?: number;
  orgId?: number;
  status?: string;
}) {
  ...
  const params: unknown[] = [];
  if (opts.dpcoOrgId) {
    conditions.push("v.dpco_org_id = ?");
    params.push(opts.dpcoOrgId);
  }
  if (opts.orgId) {
    conditions.push("v.org_id = ?");
...
async function createVerificationStatement(data: Record<string, unknown>) {
  const [result] = await q<any>(
    ...
    [
      ...
      data.filingType ?? "compliance_audit_return",
      data.filingReferenceId,
      data.organisationId,
      data.statementDate,
      data.auditScope,
      data.findingsSummary,
      data.complianceScore,
      JSON.stringify(data.nonConformities ?? []),
      JSON.stringify(data.correctiveActions ?? []),
      data.dpcoLicenceNumber,
      data.dpcoSignatoryName,
      data.dpcoSignatoryRole,
      "draft",
    ]
  );
  const [row] = await q(
    "SELECT * FROM dpco_verification_statements WHERE id = ?",
    [result.id]
  );
  return row;
}
...
async function submitVerificationStatement(id: number, signatureHash: string) {
  await q(
    "UPDATE dpco_verification_statements SET status='signed', signature_hash=?, submitted_at=NOW(), updated_at=NOW() WHERE id=?",
    [signatureHash, id]
  );
  const [row] = await q(
    "SELECT * FROM dpco_verification_statements WHERE id = ?",
...
async function listAuditEngagements(opts: {
  dpcoOrgId?: number;
  clientOrgId?: number;
  status?: string;
}) {
  ...
  const params: unknown[] = [];
  if (opts.dpcoOrgId) {
    conditions.push("e.dpco_org_id = ?");
    params.push(opts.dpcoOrgId);
  }
  if (opts.clientOrgId) {
    conditions.push("e.client_id = ?");
    params.push(opts.clientOrgId);
  }
  if (opts.status) {
    conditions.push("e.current_stage = ?");
    params.push(opts.status);
  }
  const where = conditions.length ? `WHERE ${conditions.join(" AND ")}` : "";
  return q(
...
async function upsertAuditEngagement(data: Record<string, unknown>) {
  if (data.id) {
    ...
    const [row] = await q("SELECT * FROM dpco_audit_engagements WHERE id = ?", [
      data.id,
    ]);
    return row;
  }
  const [result] = await q<any>(
    ...
    [
      data.dpcoOrganisationId ?? data.dpcoOrgId,
      data.clientOrganisationId ?? data.clientId,
      data.title ?? data.scope ?? "New Audit",
...
// ─── DPCO Training ────────────────────────────────────────────────────────────
async function listTrainingSessions(opts: {
  dpcoOrgId?: number;
  clientOrgId?: number;
  status?: string;
}) {
...
async function upsertTrainingSession(data: Record<string, unknown>) {
  if (data.id) {
    await q(
      ...
      [
        ...
        data.trainerName,
        data.description,
        data.ndpcAccredited ? 1 : 0,
        data.cpeCredits ?? 0,
        data.id,
      ]
    );
    const [row] = await q("SELECT * FROM dpco_training_sessions WHERE id = ?", [
      data.id,
    ]);
    return row;
  }
  const [result] = await q<any>(
    ...
    [
      data.dpcoId ?? 0,
      data.dpcoOrganisationId,
      data.clientOrganisationId,
      data.title,
      data.trainingType ?? "ndpa_overview",
...
async function enrollParticipant(data: Record<string, unknown>) {
  ...
  await q(
    "UPDATE dpco_training_sessions SET enrolled_count = enrolled_count + 1 WHERE id = ?",
    [data.sessionId]
  );
  const [row] = await q(
    "SELECT * FROM dpco_training_participants WHERE id = ?",
    [result.id]
  );
  return row;
}
...
// ─── DPCO Policy Drafts ───────────────────────────────────────────────────────
async function listPolicyDrafts(opts: {
  dpcoOrgId?: number;
  clientOrgId?: number;
  docType?: string;
  status?: string;
  ...
  if (opts.clientOrgId) {
    conditions.push("p.client_organisation_id = ?");
    params.push(opts.clientOrgId);
  }
  if (opts.docType) {
...
async function upsertPolicyDraft(data: Record<string, unknown>) {
  if (data.id) {
    ...
    const [row] = await q("SELECT * FROM dpco_policy_drafts WHERE id = ?", [
      data.id,
    ]);
    return row;
  }
...
async function getDpcoDashboardStats(dpcoOrgId?: number) {
  ...
  const [totalDpcos] = await q<{ c: number }>(
    "SELECT COUNT(*) as c FROM dpco_organisations"
  );
  const [activeDpcos] = await q<{ c: number }>(
    "SELECT COUNT(*) as c FROM dpco_organisations WHERE status = 'active'"
  );
  const [expiredDpcos] = await q<{ c: number }>(
    "SELECT COUNT(*) as c FROM dpco_organisations WHERE status = 'expired'"
  );
  const [expiringDpcos] = await q<{ c: number }>(
    "SELECT COUNT(*) as c FROM dpco_organisations WHERE status = 'active' AND licence_expires_at BETWEEN CURRENT_DATE AND CURRENT_DATE + INTERVAL '90 days'"
  );
  const clientFilter = dpcoOrgId
    ? `WHERE dpco_org_id = ? AND status = 'active'`
    : `WHERE status = 'active'`;
  const [activeClients] = await q<{ c: number }>(
    `SELECT COUNT(*) as c FROM dpco_clients ${clientFilter}`,
    params
  );
  const engFilter = dpcoOrgId
    ? `WHERE dpco_org_id = ? AND current_stage NOT IN ('car_filed','report_issued')`
    : `WHERE current_stage NOT IN ('car_filed','report_issued')`;
  const [pendingCars] = await q<{ c: number }>(
    `SELECT COUNT(*) as c FROM dpco_audit_engagements ${engFilter}`,
    params
  );
  const genFilter = dpcoOrgId ? `WHERE dpco_org_id = ?` : ``;
  const [trainingSessions] = await q<{ c: number }>(
    `SELECT COUNT(*) as c FROM dpco_training_sessions ${genFilter}`,
    params
  );
  const [verificationStatements] = await q<{ c: number }>(
    `SELECT COUNT(*) as c FROM dpco_verification_statements ${genFilter}`,
    params
  );
  const [policyDrafts] = await q<{ c: number }>(
    `SELECT COUNT(*) as c FROM dpco_policy_drafts ${genFilter}`,
    params
  );
...
const dpcoOrgInput = z.object({
  ...
  status: z
    ...
    .optional(),
  organisationType: z
    .enum(["law_firm", "it_provider", "audit_firm", "consultancy", "other"])
    .optional(),
...
const policyDraftInput = z.object({
  ...
  title: z.string().min(2).max(512),
  status: z
    .enum(["draft", "review", "approved", "signed", "published", "archived"])
    .optional(),
...
export const dpcoRouter = router({
  ...
  listClients: protectedProcedure
    ...
    .query(async ({ input }) =>
      listDpcoClients(input?.dpcoOrgId, input?.status)
    ),
  ...
  submitVerificationStatement: protectedProcedure
    ...
    .mutation(async ({ input }) =>
      submitVerificationStatement(input.id, input.signatureHash)
    ),
  ...
  listParticipants: protectedProcedure
    ...
    .query(async ({ input }) =>
      q(
        "SELECT * FROM dpco_training_participants WHERE session_id = ? ORDER BY enrolled_at DESC",
        [input.sessionId]
      )
    ),
  ...
  // Analytics and notifications are authoritative only when their assigned service responds.
  // Production must configure DPCO_*_URL; routes fail visibly rather than returning a
  // plausible database projection or empty notification set on a service outage.

  analyticsComplianceTrends: protectedProcedure.query(async () => {
    const baseUrl = process.env.DPCO_ANALYTICS_URL;
    if (!baseUrl)
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
        message: "DPCO analytics service is not configured",
      });
    try {
      const res = await fetch(`${baseUrl}/api/dpco/analytics/trends`, {
      ...
      logger.warn(
        ...
        "DPCO analytics trends service unavailable"
      );
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
        message: "DPCO analytics service is unavailable",
      });
    }
  }),

  analyticsPortfolio: protectedProcedure.query(async () => {
    const baseUrl = process.env.DPCO_ANALYTICS_URL;
    if (!baseUrl)
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
    ...
    try {
      const res = await fetch(`${baseUrl}/api/dpco/analytics/portfolio`, {
        signal: AbortSignal.timeout(5000),
      });
      if (!res.ok) throw new Error(`analytics status ${res.status}`);
      return res.json();
      ...
      logger.warn(
        { err: e instanceof Error ? e.message : String(e) },
        "DPCO analytics portfolio service unavailable"
      );
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
        message: "DPCO analytics service is unavailable",
  ...
  analyticsHeatmap: protectedProcedure.query(async () => {
    const baseUrl = process.env.DPCO_ANALYTICS_URL;
    if (!baseUrl)
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
    ...
    try {
      const res = await fetch(`${baseUrl}/api/dpco/analytics/heatmap`, {
        signal: AbortSignal.timeout(5000),
      });
      if (!res.ok) throw new Error(`analytics status ${res.status}`);
      return res.json();
      ...
      logger.warn(
        { err: e instanceof Error ? e.message : String(e) },
        "DPCO analytics heatmap service unavailable"
      );
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
        message: "DPCO analytics service is unavailable",
  ...
  listNotifications: protectedProcedure
    ...
    .query(async ({ input }) => {
      const baseUrl = process.env.DPCO_NOTIFICATION_URL;
      if (!baseUrl)
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "DPCO notification service is not configured",
        });
      const url = new URL(`${baseUrl}/api/dpco/notifications`);
      ...
      try {
        ...
        logger.warn(
          { err: e instanceof Error ? e.message : String(e) },
          "DPCO notification service unavailable"
        );
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "DPCO notification service is unavailable",
        });
      }
    }),
  ...
  sendNotification: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const baseUrl = process.env.DPCO_NOTIFICATION_OUTBOX_URL;
      const internalToken = process.env.DPCO_NOTIFICATION_INTERNAL_TOKEN;
      const tenantId = ctx.user.organizationId ?? ctx.user.dpcoOrgId;
      if (!baseUrl || !internalToken)
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "DPCO durable notification service is not configured",
        });
      if (!tenantId)
        throw new TRPCError({
          code: "FORBIDDEN",
          message:
            "An organization-scoped DPCO identity is required to send notifications",
        });
      try {
        const res = await fetch(`${baseUrl}/api/dpco/notifications/send`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "Idempotency-Key": input.idempotencyKey,
            "X-NDSEP-Internal-Token": internalToken,
            "X-NDSEP-Tenant-ID": String(tenantId),
            "X-NDSEP-Actor-ID": String(ctx.user.id),
          },
          body: JSON.stringify({
        ...
        if (res.status === 409)
          throw new TRPCError({
            code: "CONFLICT",
            message:
              "Idempotency key was previously used with a different notification intent",
          });
        if (!res.ok)
          throw new Error(`notification outbox status ${res.status}`);
        return payload;
      } catch (e: unknown) {
        if (e instanceof TRPCError) throw e;
        logger.warn(
          { err: e instanceof Error ? e.message : String(e) },
          "DPCO durable notification service unavailable"
        );
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "DPCO durable notification service is unavailable",
        });
      }
    }),
  ...
  signVerificationStatement: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        const baseUrl = process.env.DPCO_VERIFICATION_URL;
        if (!baseUrl)
          throw new TRPCError({
            code: "INTERNAL_SERVER_ERROR",
            message: "DPCO verification service is not configured",
          });
        const res = await fetch(
          `${baseUrl}/api/dpco/verification/statements/${input.statementId}/sign`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              signed_by: input.signedBy,
              user_id: input.userId ?? ctx.user.id,
            }),
        ...
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: (err as Record<string, unknown>).error
        ...
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: "dpco-verification-service unavailable",
        });
      }
    }),

  microserviceHealth: protectedProcedure.query(async () => {
    const services = [
      {
        name: "dpco-audit-service",
        port: Number(process.env.DPCO_AUDIT_PORT ?? 8300),
        stack: "Go/Temporal/Keycloak/Permify/Kafka/Redis",
      },
      {
        name: "dpco-registry-service",
        port: Number(process.env.DPCO_REGISTRY_PORT ?? 8310),
        stack: "Go/Dapr/TigerBeetle/Redis/APISIX",
      },
      {
        name: "dpco-verification-service",
        port: Number(process.env.DPCO_VERIFICATION_PORT ?? 8320),
        stack: "Go/Temporal/Permify/PKCS7/Kafka",
      },
      {
        name: "dpco-analytics-service",
        port: Number(process.env.DPCO_ANALYTICS_PORT ?? 8330),
        stack: "Python/Kafka/Fluvio/Lakehouse/Dapr",
      },
      {
        name: "dpco-notification-service",
        port: Number(process.env.DPCO_NOTIFICATION_PORT ?? 8340),
        stack: "Python/Dapr/Kafka/Fluvio/Redis",
      },
    ];
  ...
  listEvidence: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      if (input.engagement_id) {
        conditions.push("engagement_id = $" + (params.length + 1));
        params.push(input.engagement_id);
      }
      if (input.dpco_org_id) {
        conditions.push("dpco_org_id = $" + (params.length + 1));
        params.push(input.dpco_org_id);
      }
      if (input.category) {
      ...
      const [{ total }] = await q<{ total: string }>(
        `SELECT COUNT(*) as total FROM dpco_evidence_items WHERE ${where}`,
        params
      );
      return {
        rows,
  ...
  addEvidence: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const existing = await q(
        "SELECT id, file_name FROM dpco_evidence_items WHERE sha256_hash = $1",
        [input.sha256_hash]
      );
      if (existing.length > 0) {
        const dup = existing[0] as any;
        emitMutationEvent("ndsep.dpco.mutation", {
          action: "dpco",
        ...
        return {
          id: dup.id,
          duplicate: true,
          message: `Identical file already exists: ${dup.file_name}`,
        };
      }
  ...
  verifyEvidence: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const [item] = (await q(
        "SELECT * FROM dpco_evidence_items WHERE id = $1",
        [input.id]
      ...
      return {
        ...
        providedHash: input.sha256_hash,
      };
    }),
      ...
      return {
        ...
        trainingCompletion: trainingStats
          ? {
              ...
              completed: Number(trainingStats.completed),
            }
          : { total: 0, completed: 0 },
        recentBreaches,
      };
  ...
  submitRenewalApplication: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      return {
        success: true,
        message: "Renewal application submitted to NDPC for review",
      };
    }),
  deleteOrganisation: deleteProcedure
    .input(z.object({ id: z.number().int().positive() }))
    .mutation(async ({ input }: { input: { id: number } }) =>
      deleteDpcoOrganisation(input.id)
    ),
  ...
  registerOrganisation: publicProcedure
    .input(
      z.object({
        ...
        organisationType: z
          .enum(["private", "public", "ngo", "academic", "government"])
          .default("private"),
    ...
    .mutation(async ({ input }) => {
      ...
      if (existing) {
        throw new TRPCError({
          code: "CONFLICT",
          message:
            "An organisation with this email or CAC number is already registered.",
        });
      }
      ...
      return {
        ...
        provisionalLicence,
        message:
          "Your registration has been submitted. NDPC will review your application within 5-10 business days.",
      };
    }),
  ...
  listPendingRegistrations: protectedProcedure
    .input(
      z
        .object({
          status: z
            .enum(["pending", "active", "suspended", "revoked", "all"])
            .default("pending"),
          limit: z.number().int().min(1).max(200).default(50),
  ...
  approveRegistration: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const licenceDate =
        input.licenceDate ?? new Date().toISOString().split("T")[0];
      ...
      const [org] = await q<any>(
        `SELECT * FROM dpco_organisations WHERE id = ?`,
        [input.id]
      );
  ...
  saveControlRatings: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const compliantCount = input.ratings.filter(
        r => r.rating === "compliant"
      ).length;
  ...
  assignClientPolicy: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const [row] = await q<any>(
        `SELECT * FROM dpco_client_policies WHERE dpco_org_id = ? AND client_id = ? AND template_id = ?`,
        [input.dpcoOrgId, input.clientId, input.templateId]
      );
      emitMutationEvent("ndsep.dpco.mutation", {
        action: "dpco",
        ts: new Date().toISOString(),
  ...
  getEvidenceCoverage: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      for (const row of rows) {
        const ids: string[] = row.control_ids
          ? JSON.parse(row.control_ids)
          : [];
        for (const id of ids) {
  ...
  submitEngagementRequest: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const [dpco] = await q(
        "SELECT id, name FROM dpco_organisations WHERE id = ? AND status = 'active'",
        [input.dpcoOrgId]
      );
      if (!dpco)
        throw new TRPCError({
          code: "NOT_FOUND",
          message: "DPCO not found or not active",
        });
      // Generate reference token
      const token = `ENG-${Date.now().toString(36).toUpperCase()}-${Array.from(
      ...
      const [row] = await q(
        ...
        [
          input.orgName,
          input.orgSector ?? null,
          input.orgCountry ?? null,
          input.orgRegistrationNumber ?? null,
          input.contactName,
          input.contactEmail,
          input.contactPhone ?? null,
          input.dpcoOrgId,
          input.auditScope ?? null,
          ...
          input.processingActivities
            ? JSON.stringify(input.processingActivities)
            : null,
          token,
        ]
      );
  ...
  listIncomingRequests: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (!dpcoOrgId)
        throw new TRPCError({ code: "FORBIDDEN", message: "Not a DPCO user" });
      const conditions = ["dpco_org_id = ?"];
      ...
      const [{ total }] = await q<{ total: number }>(
        `SELECT COUNT(*) as total FROM dpco_engagement_requests ${where}`,
        params
      );
      return { rows, total };
    }),
  ...
  respondToEngagementRequest: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.decision === "accepted") {
        ...
        const plannedEnd = new Date(Date.now() + 90 * 24 * 60 * 60 * 1000)
          .toISOString()
          .split("T")[0];
        const [result] = await q<any>(
