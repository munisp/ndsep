...
export const dataPipelineRouter = router({
  listFlows: protectedProcedure
    .input(
      z.object({
        engine: z.enum(["nifi", "dbt", "airflow", "all"]).default("all"),
      })
    )
    .query(async ({ input }) => {
      const p = pool();
      try {
        const q =
          input.engine === "all"
            ? await p.query(
                "SELECT * FROM data_pipeline_flows ORDER BY engine, flow_name"
              )
            : await p.query(
                "SELECT * FROM data_pipeline_flows WHERE engine = $1 ORDER BY flow_name",
                [input.engine]
              );
        return q.rows;
      } finally {
    ...
    .query(async ({ input }) => {
      ...
      try {
        const q = await p.query(
          "SELECT * FROM data_pipeline_flows WHERE flow_id = $1",
          [input.flowId]
        );
  ...
  createFlow: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        emitMutationEvent("ndsep.data_pipeline.mutation", {
          action: "phase12Features",
  ...
  updateFlowStatus: adminProcedure
    .input(
      z.object({
        flowId: z.string(),
        status: z.enum(["running", "stopped", "error", "paused"]),
      })
    )
    .mutation(async ({ input }) => {
      const p = pool();
  ...
  getDbtModels: protectedProcedure.query(async () => {
    ...
    try {
      const q = await p.query(
        "SELECT * FROM dbt_models ORDER BY schema_name, model_name"
      );
      return q.rows;
    } finally {
      await p.end();
  ...
  getPipelineStats: protectedProcedure.query(async () => {
    const p = pool();
    try {
      const flows = await p.query(
        "SELECT COUNT(*) as total, SUM(CASE WHEN status='running' THEN 1 ELSE 0 END) as running, SUM(records_processed) as total_records FROM data_pipeline_flows"
      );
      const dbt = await p.query(
        "SELECT COUNT(*) as total, SUM(CASE WHEN last_run_status='success' THEN 1 ELSE 0 END) as success FROM dbt_models"
      );
      const airflow = await p.query(
        "SELECT COUNT(*) as total, SUM(CASE WHEN is_active THEN 1 ELSE 0 END) as active FROM airflow_dags"
      );
      return {
        flows: flows.rows[0],
        dbt: dbt.rows[0],
...
export const dataLineageRouter = router({
  getGraph: protectedProcedure.query(async () => {
    const p = pool();
    try {
      const nodes = await p.query(
        "SELECT * FROM data_lineage_nodes ORDER BY node_type, name"
      );
  ...
  getNode: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      try {
        const q = await p.query(
          "SELECT * FROM data_lineage_nodes WHERE node_id = $1",
          [input.nodeId]
        );
        if (!q.rows[0]) throw new TRPCError({ code: "NOT_FOUND" });
        const upstream = await p.query(
          "SELECT * FROM data_lineage_edges WHERE target_node_id = $1",
          [input.nodeId]
        );
        const downstream = await p.query(
          "SELECT * FROM data_lineage_edges WHERE source_node_id = $1",
          [input.nodeId]
        );
        return {
          node: q.rows[0],
  ...
  listEvents: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      try {
        ...
        let q =
          "SELECT cle.*, o.name as org_name FROM consent_lifecycle_events cle LEFT JOIN organizations o ON cle.org_id = o.id WHERE 1=1";
        const params: unknown[] = [];
        if (input.orgId) {
          params.push(input.orgId);
          q += ` AND cle.org_id = $${params.length}`;
        }
        if (input.eventType) {
          params.push(input.eventType);
          q += ` AND cle.event_type = $${params.length}`;
        }
        ...
        const count = await p.query(
          "SELECT COUNT(*) FROM consent_lifecycle_events"
        );
    ...
    .query(async ({ input }) => {
      ...
      try {
        ...
        if (input.itemType) {
          ...
          q += ` AND item_type = $${params.length}`;
        }
        if (input.impactLevel) {
          params.push(input.impactLevel);
          q += ` AND impact_level = $${params.length}`;
        }
        if (input.actionRequired !== undefined) {
          params.push(input.actionRequired);
          q += ` AND action_required = $${params.length}`;
        }
        if (input.search) {
          params.push(`%${input.search}%`);
          q += ` AND (title ILIKE $${params.length} OR summary ILIKE $${params.length})`;
        }
      ...
      try {
        const q = await p.query(
          ...
          [
            ...
            input.sourceUrl,
            input.sourceOrg,
            JSON.stringify(input.affectedSectors),
            JSON.stringify(input.ndpaArticles),
...
export const incidentResponseRouter = router({
  listPlaybooks: protectedProcedure.query(async () => {
    ...
    try {
      const q = await p.query(
        "SELECT * FROM incident_playbooks WHERE is_active = TRUE ORDER BY severity DESC, title"
      );
      return q.rows;
    } finally {
      await p.end();
  ...
  getPlaybook: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      try {
        const q = await p.query(
          "SELECT * FROM incident_playbooks WHERE id = $1",
          [input.id]
        );
        if (!q.rows[0]) throw new TRPCError({ code: "NOT_FOUND" });
        return q.rows[0];
    ...
    .query(async ({ input }) => {
      ...
      try {
        ...
        if (input.status) {
          ...
          q += ` AND ira.status = $${params.length}`;
        }
  ...
  listAssessments: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      try {
        ...
        if (input.orgId) {
          params.push(input.orgId);
          q += ` AND cga.org_id = $${params.length}`;
        }
  ...
  runAssessment: protectedProcedure
    .input(
      z.object({
        orgId: z.number(),
        assessmentType: z
          .enum(["ndpa_full", "gdpr_gap", "iso27001", "pcidss"])
          .default("ndpa_full"),
      })
    )
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        const ndpaGaps = [
          {
            article: "Article 24",
            gap: "Automated decision-making policy missing",
            severity: "high",
            remediation: "Implement AI governance policy",
          },
          {
            article: "Article 25",
            gap: "Consent records incomplete",
            severity: "medium",
            remediation: "Deploy consent management system",
          },
          {
            article: "Article 32",
            gap: "DPO not registered with NDPC",
            severity: "critical",
            remediation: "Register DPO within 30 days",
          },
          {
            article: "Article 40",
            gap: "Breach notification procedure untested",
            severity: "high",
            remediation: "Conduct tabletop exercise",
          },
          {
            article: "Article 41",
            gap: "Cross-border transfer records incomplete",
            severity: "medium",
            remediation: "Update transfer register",
          },
        ].slice(0, gapCount);
        ...
        const q = await p.query(
          ...
          [
            ...
            mediumGaps,
            lowGaps,
            JSON.stringify(ndpaGaps),
            JSON.stringify([
              { priority: 1, action: "Register DPO", deadline: "30 days" },
            ]),
          ]
        );
        emitMutationEvent("ndsep.data_pipeline.mutation", {
...
export const vendorRiskRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      try {
        let q =
          "SELECT *, dpa_executed as dpa_signed FROM vendor_risk_profiles WHERE 1=1";
        const params: unknown[] = [];
        if (input.orgId) {
          params.push(input.orgId);
          q += ` AND org_id = $${params.length}`;
        }
        if (input.riskLevel) {
          params.push(input.riskLevel);
          q += ` AND risk_level = $${params.length}`;
        }
  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        if (
          input.dataCategories.includes("health_data") ||
          input.dataCategories.includes("financial_data")
        )
          riskScore += 15;
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        const params: unknown[] = [];
        if (input.dpaExecuted !== undefined) {
          params.push(input.dpaExecuted);
          updates.push(`dpa_executed = $${params.length}`);
        }
        if (input.contractStatus) {
          params.push(input.contractStatus);
          updates.push(`contract_status = $${params.length}`);
        }
        if (input.riskScore !== undefined) {
          params.push(input.riskScore);
          updates.push(`risk_score = $${params.length}`);
          const level =
            input.riskScore >= 70
              ? "high"
              : input.riskScore >= 40
                ...
                : "low";
          params.push(level);
          updates.push(`risk_level = $${params.length}`);
        }
        updates.push("updated_at = NOW()");
        params.push(input.id);
        await p.query(
          `UPDATE vendor_risk_profiles SET ${updates.join(", ")} WHERE id = $${params.length}`,
          params
        );
        emitMutationEvent("ndsep.data_pipeline.mutation", {
          action: "phase12Features",
  ...
  submit: publicProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        const priority = ["data_breach", "bribery"].includes(input.category)
          ? "critical"
          : "medium";
        const q = await p.query(
        ...
        return {
          ...q.rows[0],
          message: "Report submitted successfully. Reference: " + ref,
        };
      } finally {
        await p.end();
      }
      ...
      try {
        ...
        const q = await p.query(
          ...
          [
            ...
            input.innovationType,
            JSON.stringify(input.dataTypesInvolved),
      ...
      try {
        ...
        const q = await p.query(
          ...
          [
            ...
            input.aiSystemType,
            input.riskCategory,
            input.humanOversightEnabled,
            input.dataSubjectsInformed,
          ]
        );
        emitMutationEvent("ndsep.data_pipeline.mutation", {
          action: "phase12Features",
...
export const nationalIdRouter = router({
  ...
  verify: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const p = pool();
      try {
        const correlationHeader = ctx.req.headers["x-correlation-id"];
        const correlationId = Array.isArray(correlationHeader)
          ? correlationHeader[0]
          : correlationHeader;
        let verification: NationalIdVerification;
        try {
          verification = await verifyNationalId({
            idType: input.idType,
          ...
          if (error instanceof NationalIdProviderError) {
            logger.warn(
              { kind: error.kind, orgId: input.orgId, idType: input.idType },
              "National-ID verification provider unavailable"
            );
            throw new TRPCError({
              code: "SERVICE_UNAVAILABLE",
              message:
                "National-ID verification service is unavailable; no verification result was recorded",
            });
          }
          throw error;
        }

        const ref = verification.providerReference;
        await p.query(
        ...
        emitMutationEvent("ndsep.data_pipeline.mutation", {
          action: "national_id_verification",
        ...
        return {
          ...
          verificationRef: ref,
          idType: input.idType,
          status: verification.verified ? "verified" : "not_found",
          nimcApiStatus: verification.providerStatus,
          provenance: verification.provenance,
          message: verification.verified
            ? `${input.idType.toUpperCase()} was verified by the configured NIMC NVS bridge`
            : `${input.idType.toUpperCase()} was not verified by the configured NIMC NVS bridge`,
...
export const businessRegistrationRouter = router({
  list: protectedProcedure.query(async () => {
    const p = pool();
    try {
      const result = await p.query(`
        SELECT cbv.*, o.name AS org_name
        FROM cac_business_verifications cbv
        JOIN organizations o ON o.id = cbv.organization_id
        ORDER BY cbv.last_verified_at DESC
      `);
      return result.rows;
    } finally {
      await p.end();
  ...
  verify: protectedProcedure
    .input(
      z.object({
        ...
        registrationNumber: z.string().trim().min(2).max(128),
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const correlationId = Array.isArray(correlationHeader)
        ? correlationHeader[0]
        : correlationHeader;
      let verification: CacBusinessVerification;
      try {
        verification = await verifyBusinessRegistration({
          registrationNumber: input.registrationNumber,
          purpose: input.purpose,
          correlationId,
        });
      } catch (error) {
        if (error instanceof CacProviderError) {
          logger.warn(
            { kind: error.kind, orgId: input.orgId },
            "CAC business-registration provider unavailable"
          );
          throw new TRPCError({
            code: "SERVICE_UNAVAILABLE",
            message:
              "CAC business-registration verification service is unavailable; no verification result was recorded",
          });
        }
        throw error;
      }
      ...
      const p = pool();
      try {
        const organization = await p.query(
        ...
        await p.query(
          `INSERT INTO cac_business_verifications (
             provider_reference, organization_id, registration_number,
             verification_purpose, verified, provider_status, legal_name,
             registration_type, provenance, request_count, success_count,
             failure_count, last_verified_at, updated_at
           ) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,1,$10,$11,NOW(),NOW())
           ON CONFLICT (organization_id, registration_number) DO UPDATE SET
             provider_reference = EXCLUDED.provider_reference,
             verification_purpose = EXCLUDED.verification_purpose,
             verified = EXCLUDED.verified,
             provider_status = EXCLUDED.provider_status,
             legal_name = EXCLUDED.legal_name,
             registration_type = EXCLUDED.registration_type,
             provenance = EXCLUDED.provenance,
             request_count = cac_business_verifications.request_count + 1,
             success_count = cac_business_verifications.success_count + EXCLUDED.success_count,
             failure_count = cac_business_verifications.failure_count + EXCLUDED.failure_count,
             last_verified_at = NOW(),
             updated_at = NOW()`,
          [
            ...
            verification.legalName ?? null,
            verification.registrationType ?? null,
      ...
      emitMutationEvent("ndsep.business_registration.mutation", {
        action: "cac_business_registration_verification",
        verified: verification.verified,
        provenance: verification.provenance,
        ts: new Date().toISOString(),
      }).catch((error: unknown) =>
      ...
      return {
        verified: verification.verified,
        verificationRef: verification.providerReference,
    ...
    try {
      const result = await p.query(`
        SELECT
          ...
          COALESCE(ROUND(SUM(success_count)::numeric / NULLIF(SUM(request_count), 0) * 100, 2), 0) AS success_rate,
          COUNT(DISTINCT organization_id)::int AS orgs_using
        FROM cac_business_verifications
      `);
...
export const crossAgencyRouter = router({
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        const q = await p.query(
          ...
          [
            ref,
            input.requestingAgency,
            input.providingAgency,
            JSON.stringify(input.dataCategories),
            input.legalBasis,
            input.ndpaArticle,
            input.purpose,
            input.encryptionStandard,
          ]
        );
        emitMutationEvent("ndsep.data_pipeline.mutation", {
          action: "phase12Features",
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      return {
        ...
        transfers30d: parseInt(r.transfers_30d) || 0,
      };
    } finally {
      await p.end();
    }
  ...
  suspend: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        await p.query(
          `UPDATE cross_agency_data_shares SET status = 'suspended' WHERE id = $1`,
          [input.agreementId]
        );
        emitMutationEvent("ndsep.data_pipeline.mutation", {
          action: "phase12Features",
...
export const piaRouter = router({
  list: protectedProcedure
    .input(
      z.object({ orgId: z.number().optional(), status: z.string().optional() })
    )
    .query(async ({ input }) => {
      const p = pool();
      try {
        ...
        if (input.orgId) {
          params.push(input.orgId);
          q += ` AND pa.org_id = $${params.length}`;
        }
        if (input.status) {
          params.push(input.status);
          q += ` AND pa.status = $${params.length}`;
        }
...
export const platformNotificationsRouter = router({
    ...
    .query(async ({ input, ctx }) => {
      ...
      try {
        ...
        if (input.userId) {
          ...
          q += ` AND (user_id = $${params.length} OR user_id IS NULL)`;
        }
        if (input.unreadOnly) {
          q += " AND is_read = FALSE";
        }
  ...
  getStats: protectedProcedure.query(async () => {
    ...
    try {
      ...
      return {
        ...
        pendingReview: parseInt(r.pending_review) || 0,
  ...
  markRead: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        await p.query(
          "UPDATE platform_notifications SET is_read = TRUE WHERE id = $1",
          [input.id]
        );
        emitMutationEvent("ndsep.data_pipeline.mutation", {
          action: "phase12Features",
  ...
  markAllRead: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        if (input.userId) {
          await p.query(
            "UPDATE platform_notifications SET is_read = TRUE WHERE user_id = $1 OR user_id IS NULL",
            [input.userId]
          );
        } else {
          await p.query(
            "UPDATE platform_notifications SET is_read = TRUE WHERE user_id IS NULL"
          );
        }
        emitMutationEvent("ndsep.data_pipeline.mutation", {
          action: "phase12Features",
  ...
  getUnreadCount: protectedProcedure.query(async () => {
    ...
    try {
      const q = await p.query(
        "SELECT COUNT(*) as count FROM platform_notifications WHERE is_read = FALSE"
      );
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        try {
          ...
          const intent = await stripeClient.paymentIntents.confirm(
            input.intentId,
            {
              payment_method:
                input.paymentMethodType === "card" ? "pm_card_visa" : undefined,
            }
          );
...
export const finesRouter = router({
  listFines: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      try {
        ...
        if (input.orgId) {
          params.push(input.orgId);
          q += ` AND ef.organization_id = $${params.length}`;
        }
        if (input.status) {
          params.push(input.status);
          q += ` AND ef.status = $${params.length}`;
        }
  ...
  initiatePayment: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const p = pool();
      try {
        const fine = await p.query(
          `SELECT ef.*, o.name as org_name FROM enforcement_fines ef LEFT JOIN organizations o ON ef.organization_id = o.id WHERE ef.id = $1`,
          [input.fineId]
        );
          ...
          line_items: [
            {
              price_data: {
                currency: "usd",
                product_data: {
                  name: `NDPA Fine - ${f.org_name ?? "Organisation"}`,
                  description: `Fine ID: ${input.fineId}`,
                },
        ...
        await p.query(
          `UPDATE enforcement_fines SET payment_reference = $1 WHERE id = $2`,
          [session.id, input.fineId]
        );
        emitMutationEvent("ndsep.data_pipeline.mutation", {
          action: "phase12Features",
          ts: new Date().toISOString(),
  ...
  waiveFine: adminProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        await p.query(
          `UPDATE enforcement_fines SET status = 'waived' WHERE id = $1`,
          [input.fineId]
        );
        emitMutationEvent("ndsep.data_pipeline.mutation", {
          action: "phase12Features",
...
export const phase12Router = router({
  ...
  businessRegistration: businessRegistrationRouter,
