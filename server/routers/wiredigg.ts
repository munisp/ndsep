...
const WIREDIGG_URL = process.env.WIREDIGG_URL?.trim();

async function wirediggFetch(
  path: string,
  method = "GET",
  body?: unknown
): Promise<unknown> {
  if (!WIREDIGG_URL) {
    throw new TRPCError({
      ...
      message: "WireDigger service is not configured",
    });
  }
  ...
  try {
    const res = await fetch(`${WIREDIGG_URL}${path}`, {
      ...opts,
    ...
    if (!res.ok) {
      ...
      throw new TRPCError({
        code: "SERVICE_UNAVAILABLE",
        message: `WireDigger service returned HTTP ${res.status}: ${text.slice(0, 256)}`,
      });
    }
    return res.json();
  } catch (e: unknown) {
    ...
    logger.warn(
      { err: msg, path },
      "WireDigger service unreachable; no response returned"
    );
    throw new TRPCError({
      code: "SERVICE_UNAVAILABLE",
      message: "WireDigger service is unavailable",
    });
  }
}
...
export const wirediggRouter = router({
  ...
  startCapture: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      emitMutationEvent(EVENTS.NETWORK_CAPTURE_STARTED, {
  ...
  stopCapture: protectedProcedure.mutation(async () => {
    ...
    emitMutationEvent(EVENTS.NETWORK_CAPTURE_STOPPED, { action: "stop" }).catch(
      (e: unknown) =>
        logger.debug(
          { err: e instanceof Error ? e.message : String(e) },
          "fire-and-forget failed"
  ...
  resetCapture: protectedProcedure.mutation(async () =>
    wirediggFetch("/api/capture/reset", "POST")
  ),
  captureStats: protectedProcedure.query(async () =>
    wirediggFetch("/api/capture/stats")
  ),
  ...
  threatSummary: protectedProcedure.query(async () =>
    wirediggFetch("/api/threats/summary")
  ),

  // ── Anomaly detection ──
  anomalyStats: protectedProcedure.query(async () =>
    wirediggFetch("/api/anomaly/stats")
  ),
  analyzeBatch: protectedProcedure.mutation(async () => {
    const result = await wirediggFetch("/api/anomaly/analyze", "POST");
    emitMutationEvent(EVENTS.NETWORK_ANOMALY_ANALYZED, {
  ...
  // ── IoT ──
  iotDevices: protectedProcedure.query(async () =>
    wirediggFetch("/api/iot/devices")
  ),
  iotHighRisk: protectedProcedure.query(async () =>
    wirediggFetch("/api/iot/high-risk")
  ),

  // ── Protocol & network stats ──
  protocolStats: protectedProcedure.query(async () =>
    wirediggFetch("/api/protocols")
  ),
  ...
  addMaliciousIp: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      const result = await wirediggFetch(
        "/api/threat-intel/add-ip",
        "POST",
        input
      );
      emitMutationEvent(EVENTS.NETWORK_THREAT_ADDED, { ip: input.ip }).catch(
        (e: unknown) =>
          logger.debug(
            { err: e instanceof Error ? e.message : String(e) },
            "fire-and-forget failed"
