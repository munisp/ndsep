 ...
 */
export function paramPollutionGuard(
  req: Request,
  _res: Response,
  next: NextFunction
): void {
 ...
 */
export function suspiciousRequestGuard(
  req: Request,
  res: Response,
  next: NextFunction
): void {
  ...
  if (checkValue(req.path)) {
    logger.warn(
      ...
      "[Security] Suspicious path blocked"
    );
  ...
  if (checkValue(req.originalUrl)) {
    logger.warn(
      ...
      "[Security] Suspicious URL blocked"
    );
    res.status(400).json({ error: "Invalid request" });
    return;
  }
  ...
  if (checkObject(req.query)) {
    logger.warn(
      ...
      "[Security] Suspicious query blocked"
    );
    res.status(400).json({ error: "Invalid request" });
    return;
  }
 ...
 */
export function demoLoginGuard(
  req: Request,
  res: Response,
  next: NextFunction
): void {
  // Demo identities must never exist on a production request path. A temporary
  // operational access requirement belongs in the real identity provider and
  // audited break-glass workflow, not in a demo-session switch.
  if (process.env.NODE_ENV === "production") {
    logger.warn(
      { ip: req.ip, ua: req.get("user-agent") },
      "[Security] Demo login blocked in production"
    );
 ...
 */
export function strictJsonLimit(
  req: Request,
  res: Response,
  next: NextFunction
): void {
  ...
  if (contentLength > MAX_JSON_BYTES) {
    res
      .status(413)
      .json({ error: "Request entity too large. Maximum 1MB for API calls." });
    return;
  }
  next();
}
...
export function calculateSecurityScore(
  ...
  return {
    ...
    grade,
    findings,
    fixedCount: findings.filter(
      f => f.status === "fixed" || f.status === "mitigated"
    ).length,
...
export function requestIdMiddleware(
  ...
  const requestId = existingId?.match(/^[a-zA-Z0-9\-]{8,64}$/)
    ? existingId
    : randomUUID();
...
const authFailureStore = new Map<
  string,
  { count: number; firstSeen: number; alerted: boolean }
>();
const AUTH_FAILURE_WINDOW_MS = parseInt(
  process.env.AUTH_FAILURE_WINDOW_MS ?? "300000",
  10
); // default 5 minutes
const AUTH_FAILURE_THRESHOLD = parseInt(
  process.env.AUTH_FAILURE_THRESHOLD ?? "10",
  10
);

export function authFailureTracker(
  req: Request,
  res: Response,
  next: NextFunction
): void {
  res.on("finish", () => {
    ...
    const isAuthPath =
      ...
      req.path.includes("/demo-login") ||
      req.path.includes("/trpc/auth");
    ...
    if (entry.count >= AUTH_FAILURE_THRESHOLD && !entry.alerted) {
      entry.alerted = true;
      logger.warn(
        { ip, count: entry.count, window: "5min" },
      ...
      import("./_core/notification")
        .then(({ notifyOwner }) => {
          notifyOwner({
            ...
            content:
              `IP address ${ip} has triggered ${entry.count} authentication failures within 5 minutes. ` +
...
export async function purgeOldAuditLogs(
  ...
  try {
    ...
    if (purged > 0) {
      logger.info(
        { purged, retentionYears },
        "[Security] Audit log retention policy: purged old records"
      );
    }
    return { purged };
  } catch (err) {
 ...
 */
const STAGING_ALLOWED_IPS = (
  process.env.DEMO_LOGIN_ALLOWED_IPS ?? "127.0.0.1,::1,::ffff:127.0.0.1"
)
  .split(",")
  .map(s => s.trim());

export function enhancedDemoLoginGuard(
  req: Request,
  res: Response,
  next: NextFunction
): void {
  ...
  if (env === "staging") {
    ...
    const allowed = STAGING_ALLOWED_IPS.some(allowedIp =>
      clientIp.includes(allowedIp)
    );
    if (!allowed) {
      logger.warn(
        { ip: clientIp, allowed: STAGING_ALLOWED_IPS },
        "[Security] Demo login blocked in staging — IP not in allowlist"
      );
      res
        .status(403)
        .json({
          error: "Demo login is restricted to authorised IPs in staging",
        });
      return;
    }
  }
