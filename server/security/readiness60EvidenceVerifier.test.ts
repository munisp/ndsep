...
async function writeEvidence(directory: string, filename: string, value: unknown) {
...
async function writeValidEvidence(directory: string) {
  ...
  await writeEvidence(directory, "artifact-evidence.json", {
    sbom: { imageDigest: DIGEST, verified: true, uri: "s3://evidence/sbom.cdx.json", format: "CycloneDX" },
    provenance: { imageDigest: DIGEST, verified: true, uri: "s3://evidence/provenance.json", format: "SLSA" },
    cosign: {
      imageDigest: DIGEST,
      verified: true,
  ...
  const services = [
    ...
    "keycloak", "openappsec", "permify", "opensearch", "fluvio",
  ].map((name) => ({
    name,
    status: "passed",
    digest: DIGEST,
  ...
  await writeEvidence(directory, "postgres-integrity.json", {
    status: "passed",
    checks: [
      "encrypted-backup-restore", "forced-rls-denial", "ledger-recompute", "advisory-lock-concurrency",
    ].map((name) => ({ name, status: "passed" })),
  });
  await writeEvidence(directory, "resilience.json", {
    status: "passed",
    checks: ["stateful-service-fault-recovery", "alert-escalation", "reconciliation"].map((name) => ({ name, status: "passed" })),
  });
  await writeEvidence(directory, "residency-evidence.json", {
    status: "passed",
    checks: ["signed-evidence-verification", "replay-rejection", "merkle-transparency", "appeal-workflow"].map((name) => ({ name, status: "passed" })),
  });
  await writeEvidence(directory, "approvals.json", {
    candidateDigest: DIGEST,
    acceptances: ["release-security", "sre", "data-owner", "compliance-evidence-verifier", "security"].map((role) => ({ role, decision: "accepted" })),
  });
}
...
describe("60-point readiness evidence verifier", () => {
  ...
  it("fails closed when an SBOM format is not explicitly accepted", async () => {
    const directory = await createEvidenceDirectory();
    await writeValidEvidence(directory);
    await writeEvidence(directory, "artifact-evidence.json", {
      sbom: { imageDigest: DIGEST, verified: true, uri: "s3://evidence/sbom.unknown.json", format: "UNKNOWN" },
      provenance: { imageDigest: DIGEST, verified: true, uri: "s3://evidence/provenance.json", format: "SLSA" },
      cosign: { imageDigest: DIGEST, verified: true, uri: "s3://evidence/cosign.json", certificateIdentity: "https://github.com/munisp/ndsep/.github/workflows/ci.yml@refs/heads/production" },
    });
    const result = verifyReadiness60Evidence(directory);
    expect(result.status).toBe("blocked");
    expect(result.errors).toContain("artifact-evidence.json: an SPDX or CycloneDX SBOM is required");
  });
  ...
  it("fails closed when governance does not require code-owner review", async () => {
    ...
    const result = verifyReadiness60Evidence(directory);
    expect(result.status).toBe("blocked");
    expect(result.errors).toContain("governance.json: required CODEOWNER review is not enabled");
  });
  ...
  it("fails closed when the direct image scan contains a critical vulnerability", async () => {
    ...
    await writeEvidence(directory, "trivy.json", {
      ...
      mode: "direct-image",
      scanTarget: IMAGE,
      results: [{ vulnerabilities: [{ vulnerabilityId: "CVE-2026-0001", severity: "CRITICAL" }] }],
    });

    const result = verifyReadiness60Evidence(directory);
    expect(result.status).toBe("blocked");
    expect(result.errors).toContain("trivy.json: found 1 HIGH/CRITICAL vulnerability finding(s)");
  });
});
