...
import { assignFieldEvidenceSupervisor, escalateFieldEvidence, listFieldEvidence, recordFieldEvidence, reviewFieldEvidence } from "../server/fieldEvidenceRepository";
...
describe("field evidence repository", () => {
  it("records an offline manifest once and preserves its unverified trust state", () => {
    ...
    expect(escalateFieldEvidence({ id: input.id, escalatedBy: "admin-subject" })).toMatchObject({ status: "escalated", evidence: { verificationState: "unverified", escalatedBy: "admin-subject" } });
