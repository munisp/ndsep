...
import { resetPaymentAuditForTests, reviewOfflinePayment, submitOfflinePayment } from "../server/offlinePaymentRepository";
...
describe("PostgreSQL payment audit integrity", () => {
  it("writes an ordered submission, decision, and alert trail that cannot be edited or deleted", async () => {
    const payment = await submitOfflinePayment({ applicantOpenId: "applicant-a", applicantName: "Amina Musa", reference: "FCT-COO-1001", amountKobo: 1250000, service: "Certificate of Occupancy statutory fee", evidenceDescription: "Transfer advice supplied for review." });
    await reviewOfflinePayment({ paymentId: payment.id, decision: "approved", reviewerOpenId: "admin-a", reason: "Evidence reconciled against the administrative review record." });
    ...
    expect(events.rows.map((event) => event.event_type).sort()).toEqual(["offline_payment_approved", "offline_payment_submitted", "payment_alert_created"].sort());
