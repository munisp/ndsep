...
import { getPendingStakeholderSyncItems } from "../lib/stakeholder-sync-queue";
...
describe("stakeholder offline synchronization queue", () => {
  ...
  it("reports no items when no stakeholder payloads have been queued", async () => {
    ...
    await expect(getPendingStakeholderSyncItems()).resolves.toEqual([]);
  });

  it("returns only recognizable profile or document queue records", async () => {
    storage.getItem.mockResolvedValue(JSON.stringify([
      { id: "profile-1", kind: "profile", label: "Amina Bello Enterprises", queuedAt: "2026-08-17T12:00:00.000Z", status: "pending" },
      { ignored: true },
    ]));
    await expect(getPendingStakeholderSyncItems()).resolves.toEqual([
      { id: "profile-1", kind: "profile", label: "Amina Bello Enterprises", queuedAt: "2026-08-17T12:00:00.000Z", status: "pending" },
